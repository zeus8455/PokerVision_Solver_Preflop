...
def test_v2_0_runtime_source_switch_is_disabled_by_default_and_embedded():
    text = DISPLAY.read_text(encoding="utf-8")

    assert "V20_USE_SOLVER_PREFLOP_AS_RUNTIME_SOURCE = False" in text
    assert "V20_SOLVER_PREFLOP_DRY_RUN_ONLY = True" in text
    assert "def _select_v20_runtime_action_decision_state(" in text
    assert '"reason": "v20_switch_disabled"' in text
